...
function buildCodeReviewWorkflow(): string {
  const lines = [
    ...
    '            echo "✔ Resolved PR #${PR_NUMBER} at ${HEAD_SHA:0:12} (workflow_dispatch)"',
    '          else',
    ...
    '            echo "::error::SHA discipline violation: HEAD (${ACTUAL_SHA}) ≠ expected (${HEAD_SHA})"',
    '            exit 1',
    '          fi',
    '',
    ...
    '              core.info(`Tier 1 skip notification already posted for SHA ${headSha.slice(0, 12)} — skipping.`);',
    '              return;',
    '            }',
    '',
    ...
    '              body: [',
    '                marker,',
    '                `## ℹ️ Review Agent — Skipped (Tier 1)`,',
    "                '',",
    '                `**Commit**: \\`${headSha.slice(0, 12)}\\``,',
    '                `**Tier**: 1 (low risk — documentation / non-code changes)`,',
    "                '',",
    "                'This pull request only touches **Tier 1** files (documentation, configuration cosmetics, non-code assets). The automated code review agent does not run for Tier 1 changes — they are considered low-risk and only require lint checks.',",
    "                '',",
    "                'If this PR should be reviewed, you can manually trigger the review workflow from the **Actions** tab.',",
    "                '',",
    "                '---',",
    "                '*🤖 [Code Review Agent](https://github.com/codefactory) — automated code review.*',",
    "              ].join('\\n'),",
    '            });',
    '',
    ...
    '            } else {',
    '              core.info(`No existing review for SHA ${headSha.slice(0, 12)} — proceeding.`);',
    ...
    "              '',",
    '              `**Commit**: \\`${headSha.slice(0, 12)}\\``,',
    "              `**Status**: ${reviewOutcome === 'success' ? '✅ Complete' : '⚠️ Review may be incomplete'}`,",
    '            ];',
    '',
    ...
    '            - REQUEST_CHANGES: The review found one or more blocking issues (security vulnerabilities, bugs that cause runtime errors, data loss, or crashes). Only blocking issues warrant REQUEST_CHANGES — warnings and suggestions do not.',
    ...
    '            echo "::notice::Verdict classifier failed — defaulting to COMMENT."',
    ...
    '          fi',
    '',
    '          # Default to COMMENT if extraction failed (safe — no automated action)',
    '          if [[ -z "$VERDICT" || "$VERDICT" == "null" ]]; then',
    '            VERDICT="COMMENT"',
    '            REASON="Could not extract verdict"',
    '            echo "::notice::Could not extract verdict from classifier — defaulting to COMMENT."',
    '          fi',
    '',
    ...
    '            if (statusIdx !== -1) {',
    "              const verdictLine = `**Verdict**: ${verdictEmoji} ${verdict}${reason ? ` — ${reason}` : ''}`;",
    ...
    "              core.info('PR does not have agent-pr label — skipping review-fix dispatch.');",
    ...
    "              core.info('PR already has agent:needs-judgment label — skipping.');",
    "              core.setOutput('eligible', 'false');",
    ...
    '              issue_number: prNumber,',
    '              body: [',
    "                '## ⚠️ Review-Fix Loop — Max Cycles Reached',",
    "                '',",
...
function buildRerunWorkflow(): string {
  const lines = [
    ...
    '',
    '            if (alreadyRequested) {',
    '              core.info(`Rerun already requested for SHA ${headSha.slice(0, 12)} — skipping.`);',
    '              return;',
    '            }',
    '',
...
function buildAutoResolveWorkflow(): string {
  const lines = [
    ...
    "              // (GitHub doesn't expose resolved state via REST — use GraphQL)",
