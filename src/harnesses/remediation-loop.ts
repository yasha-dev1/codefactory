...
function buildRemediationWorkflow(testCmd: string, lintCmd: string): string {
  const lines = [
    ...
    '            echo "::error::SHA discipline violation: PR HEAD (${CURRENT}) ≠ expected (${EXPECTED}). Branch was updated since dispatch."',
    '            exit 1',
    '          fi',
    '          echo "✔ SHA verified: ${CURRENT:0:12}"',
    '',
    ...
    '            echo "::error::Post-checkout SHA mismatch: ${ACTUAL} ≠ ${EXPECTED}"',
    '            exit 1',
    '          fi',
    '',
    '      - name: Setup Node.js',
    ...
    "              '',",
    "              '---',",
    "              '*🤖 [Remediation agent](https://github.com/codefactory) — auto-fix blocked.*',",
    '            );',
    '',
    ...
    '          fi',
    '',
    '      - name: Run validation — Type Check',
    "        if: steps.changes.outputs.has-changes == 'true' && steps.protected.outputs.has-remaining == 'true'",
    '        id: typecheck',
    '        run: |',
    ...
    '          else',
    '            echo "passed=false" >> "$GITHUB_OUTPUT"',
    '            echo "::warning::Validation failed — reverting all remediation changes."',
    '            git checkout -- .',
    '          fi',
    '',
    ...
    '          echo "✔ Pushed remediation commit: ${NEW_SHA:0:12}"',
    '',
    ...
    '',
    "            const statusEmoji = validationPassed && newSha ? '✅' : hasChanges ? '⚠️' : '💬';",
    ...
    '              `|-------|--------|`,',
    "              `| Lint | ${lintOk ? '✅' : '❌'} |`,",
    "              `| Type Check | ${typecheckOk ? '✅' : '❌'} |`,",
    "              `| Tests | ${testOk ? '✅' : '❌'} |`,",
    "            ].join('\\n') : '';",
    '',
    '            const body = [',
    '              marker,',
    '              `## ${statusEmoji} Remediation Agent — ${statusText}`,',
    "              '',",
    '              `**Attempt**: ${attempt}`,',
    ...
    "              '',",
    "              '---',",
    "              '*🤖 [Remediation agent](https://github.com/codefactory) — automated code fix.*',",
    "            ].filter(Boolean).join('\\n');",
    '',
    '            await github.rest.issues.createComment({',
    '              owner: context.repo.owner,',
